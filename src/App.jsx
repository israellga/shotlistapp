...
function Field({ label, value, onChange, placeholder, mono, multiline, style }) {
        ...
        <div style={{ padding: "0 16px 18px", borderTop: `1px solid ${C.lineSoft}` }}>
          {!fieldMode && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 16, marginBottom: 6 }}>
              ...
              <Field label="Tipo de produção" value={shot.tipo} onChange={(v) => onChange({ ...shot, tipo: v })} placeholder="Reels, foto, stories..." />
...
function EquipeSection({ equipe, onChange }) {
  function update(id, patch) { onChange(equipe.map((m) => (m.id === id ? { ...m, ...patch } : m))); }
  function add() { onChange([...equipe, { id: uid(), funcao: "", responsavel: "" }]); }
    ...
    <div>
      {equipe.map((m) => (
        <div key={m.id} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}>
          <input value={m.funcao} onChange={(e) => update(m.id, { funcao: e.target.value })} placeholder="Função" style={inputInlineStyle(1)} />
          <input value={m.responsavel} onChange={(e) => update(m.id, { responsavel: e.target.value })} placeholder="Responsável" style={inputInlineStyle(1)} />
...
function CronogramaSection({ cronograma, onChange }) {
  ...
  const sorted = [...cronograma].sort((a, b) => a.horario.localeCompare(b.horario));
  return (
    <div>
      {sorted.map((c) => (
        <div key={c.id} style={{ display: "grid", gridTemplateColumns: "72px 1.4fr 1fr 1fr 30px", gap: 8, marginBottom: 8, alignItems: "center" }}>
          <input value={c.horario} onChange={(e) => update(c.id, { horario: e.target.value })} placeholder="10:30" style={{ ...inputInlineStyle(1), fontFamily: FONT_MONO, color: C.amber, textAlign: "center" }} />
...
function ProductionDetail({ production, fieldMode, onChange, onBack }) {
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 4px 60px" }}>
          ...
          {fieldMode ? (
            <>
              ...
              <div style={{ fontFamily: FONT_MONO, fontSize: 12, color: C.faint }}>
                {production.data}{totalTakes > 0 && <span> · {doneTakes}/{totalTakes} takes concluídos</span>}
              </div>
            </>
          ) : (
            <input
      ...
      {!fieldMode && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 18 }}>
          <Field label="Data" value={production.data} onChange={(v) => patch({ data: v })} placeholder="02/09/2026" mono />
          <Field label="Responsável" value={production.responsavel} onChange={(v) => patch({ responsavel: v })} placeholder="Israel" />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 12 }}>
        {!fieldMode && (
          <Section title="Equipe" icon={<Users size={15} color={C.faint} />} count={production.equipe.length}>
            <EquipeSection equipe={production.equipe} onChange={(equipe) => patch({ equipe })} />
          </Section>
        )}
        {!fieldMode && (
    ...
    <div onClick={onOpen} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: "16px 18px", cursor: "pointer", display: "flex", alignItems: "center", gap: 14 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12.5, color: C.faint, marginTop: 3, fontFamily: FONT_MONO }}>
          {p.data || "sem data"} · {p.shots.length} shot{p.shots.length !== 1 ? "s" : ""}{totalTakes > 0 && ` · ${pct}% concluído`}
        </div>
      </div>
      ...
      {!current ? (
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 4px" }}>
          <div style={{ display: "flex", alignItems: "center", marginBottom: 14 }}>
            ...
            <button onClick={addProduction} style={{ display: "flex", alignItems: "center", gap: 6, background: C.amberDim, border: `1px solid ${C.amber}`, borderRadius: 8, padding: "8px 14px", color: C.amber, fontSize: 13.5, fontWeight: 600, cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {order.length === 0 && (
              <div style={{ color: C.faint, fontSize: 13.5, padding: "40px 4px", textAlign: "center" }}>
                Nenhuma produção ainda.<br />Crie a primeira pra montar cronograma, equipe e shotlist.
              </div>
            )}
            {order.map((id) => {
